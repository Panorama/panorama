...
QVariant ApplicationFilterMethods::inCategory(QAbstractItemModel *source, const QString &category)
{
...
QVariant ApplicationFilterMethods::matching(QAbstractItemModel *source, const QString &role, const QString &value)
{
    ApplicationFilterModel *result = new ApplicationFilterModel(source, source);
    result->setParent(source);
...
QVariant ApplicationFilterMethods::sortedBy(QAbstractItemModel *source, const QString &role, bool ascending)
{
    ApplicationFilterModel *result = new ApplicationFilterModel(source, source);
    result->setParent(source);
    result->setSortRole(source->roleNames().key(role.toLocal8Bit(), ApplicationModel::Name));
