...
{
public:
    /** Finds a named icon. Will return immediately if the icon is resolved already */
...
private:
    ...
    static QList<QDir> indexForTheme(const QString &name, const QStringList &paths,
                                     const QStringList &suffixes);
    static QList<QDir> indexAllThemes(const QString &name, const QStringList &paths,
                                      const QStringList &filters);
    static void indexTheme(const QString &name, const QStringList &paths,
                           const QStringList &filters, QList<QDir> &result);
    static void indexSubdir(const QDir &dir, const QStringList &filters, QList<QDir> &result);
